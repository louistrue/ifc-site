...
| `--building-separated-elements` | Export building roofs/facades/footprints as separate surface objects | False |
...
**Export building roofs/facades/footprints as separate surfaces:**
```bash
python combined_terrain.py --egrid CH999979659148 --include-buildings --building-separated-elements --output separated_elements.ifc
```
...
When `--building-separated-elements` is set, roofs, facades, and footprints are emitted as separate `IfcGeographicElement` surface objects (SurfaceModel representations). This mirrors the swissBUILDINGS3D “separated elements” model shown in the swisstopo documentation.
