...
| `--building-mode` | Building export: `solid`, `separated`, or `both` | `solid` |
...
**Use separated elements (roof, facades, footprint) instead of solids:**
```bash
python combined_terrain.py --egrid CH999979659148 --include-buildings --building-mode separated --output separated_buildings.ifc
```
...
Set `--building-mode separated` to emit “separated elements” (roofs, facades, footprints) as individual surface objects. Use `--building-mode both` to export the surface model and an extruded solid together.
