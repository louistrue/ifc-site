...
def prepare_building_geometries(building_features, terrain_coords, terrain_elevations):
    ...
    for feature in building_features:
        ...
        part_hint = (attrs.get("objektart") or attrs.get("art") or attrs.get("type") or attrs.get("typ") or
                     attrs.get("teil") or attrs.get("klasse") or attrs.get("class") or attrs.get("form") or "")
        part_hint_upper = str(part_hint).upper()
        if "ROOF" in part_hint_upper or "DAC" in part_hint_upper or "TOIT" in part_hint_upper:
            element_type = "ROOF"
        elif "FACADE" in part_hint_upper or "WALL" in part_hint_upper or "FASSADE" in part_hint_upper:
            element_type = "FACADE"
        elif "FOOTPRINT" in part_hint_upper or "BODEN" in part_hint_upper or "GROUND" in part_hint_upper:
...
def _infer_ifc_surface_name(element_type):
    """Map element type to a descriptive IFC name suffix."""
    mapping = {
        "ROOF": "RoofSurface",
        "FACADE": "FacadeSurface",
        "FOOTPRINT": "FootprintSurface",
    }
    return mapping.get(element_type, "BuildingSurface")
...
def create_combined_ifc(terrain_triangles, site_solid_data, output_path, bounds, 
    ...
    if buildings:
        ...
        for idx, building in enumerate(buildings, 1):
            ...
            def _add_surface_representation(target_elem, name_suffix, geometry_json, base_z_value):
                faces_local = _create_faces_from_geojson(
                    model, geometry_json, offset_x, offset_y, offset_z, base_z_value
                )
                if not faces_local:
                    return False
                shell = model.createIfcOpenShell(faces_local)
                shell_model = model.createIfcShellBasedSurfaceModel([shell])
                shape_rep_local = model.createIfcShapeRepresentation(
                    body_context, "Body", "SurfaceModel", [shell_model]
                )
                target_elem.Name = f"{building_name}_{name_suffix}"
                target_elem.Representation = model.createIfcProductDefinitionShape(None, None, [shape_rep_local])
                return True

            if building_mode in ("solid", "both") and not building.get("has_z", False):
                polygons = []
            ...
            elif building_mode in ("solid", "both") and building.get("has_z"):
                if _add_surface_representation(building_elem, "SurfaceModel", building["geometry_json"], building.get("base_z", 0.0)):
                    created_any = True
            elif building_mode == "separated" and building.get("has_z"):
                created_any = _add_surface_representation(
                    building_elem, _infer_ifc_surface_name(building.get("element_type")), building["geometry_json"],
                    building.get("base_z", 0.0)
                )

            # If separated mode without Z but we can still extrude as fallback
            if not created_any and building_mode == "separated":
                polygons = []
                if isinstance(building.get("shape"), Polygon):
                    polygons = [building["shape"]]
                elif isinstance(building.get("shape"), MultiPolygon):
                ...
                if rep_items:
                    ...
                    building_elem.Name = f"{building_name}_{_infer_ifc_surface_name(building.get('element_type'))}"
                    rep = model.createIfcProductDefinitionShape(None, None, [shape_rep])
                    created_any = True
                    print(f"  Building {idx}: fallback extruded separated element with {total_faces} faces")
            ...
            if building.get("element_type"):
                pset_props["ElementType"] = building["element_type"]
...
def main():
    ...
    parser.add_argument("--building-mode", choices=["solid", "separated", "both"], default="solid",
                        help="Export buildings as extruded solids, separated surfaces, or both")
    ...
    create_combined_ifc(
        ...
        buildings=buildings_prepared, building_mode=args.building_mode
    )
