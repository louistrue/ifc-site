...
def create_combined_ifc(terrain_triangles, site_solid_data, output_path, bounds, 
    ...
    if buildings:
        ...
        for idx, building in enumerate(buildings, 1):
            ...
            def _place_and_assign(product):
                product.OwnerHistory = owner_history
                product.ObjectPlacement = base_local
                ifcopenshell.api.run(
                    "spatial.assign_container", model, products=[product], relating_structure=site
                )
            ...
            if building.get("has_z") and building.get("separate_elements"):
                ...
                for cls_name, cls_faces in grouped_faces.items():
                    if not cls_faces:
                        continue
                    shell = model.createIfcOpenShell(cls_faces)
            ...
            _place_and_assign(building_elem)
            ...
            if rep:
                ...
                _attach_metadata(building_elem)
            else:
